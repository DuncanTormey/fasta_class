...
class Fasta_file(object):
    ...
    def __init__(self, path, unmask=False):
        self.path = path
    ...
    def size_filter_tuples(self, min_length, max_length=10000000000, inplace=False):
        min_length = int(min_length)
        size_filt_tuples = []
        for i in np.arange(len(self.fasta_tuples)):
            if len(self.fasta_tuples[i][1]) > min_length and len(self.fasta_tuples[i][1]) < max_length:
                size_filt_tuples.append(self.fasta_tuples[i])

        if inplace:
            self.fasta_tuples = size_filt_tuples
            self.fasta_dict = dict(self.fasta_tuples)
            return self
        else:
            return size_filt_tuples

    def ret_total_length(self):
        total_length = sum(len(seq) for seq in self.fasta_dict.values())
        print('Total Length is: %d' % total_length)
        return total_length
